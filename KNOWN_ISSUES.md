...
| 3 | [#3](https://github.com/GISBoost/easy-R5/issues/3) | The dead-date guard (`ALLOW_NO_SERVICE`) reads `network.json`'s `service_days`, which `gtfs_calendar.compute_service_days` caps at 90 days from the **earliest** calendar entry across **all** feeds in the build folder — not per-feed, not centered on the routing date. A network combining a feed with an old calendar start (e.g. a rail feed starting many months earlier) with feeds relevant to a much later date gets a capped summary that misses the real date entirely, so the guard false-positives "no active trips" even though every feed genuinely has service that day. | `ALLOW_NO_SERVICE=True`, only after independently confirming (outside the plugin) that every feed has real active trips on the target date — this also disables the guard for an actually-serviceless day on the same network. | Known |
